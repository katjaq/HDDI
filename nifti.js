...
(function() {
	window.Nifti = function () {
		...
		this.makeNew = function( nx, ny, nz, dx, dy, dz, nt, datatype ) {
			...
			data.setUint8( 39, 1 ); //toDo: realValue? to replace 1
		...
		this.getImageRGBByte = function(orient, pos) {
			...
			if ( orient === 'sagittal' ) {
				for( var y = 0; y < dimY; ++y )
		            for( var z = 0; z < dimZ; ++z )
		            {
		                var r = data.getUint8( this.getId(pos-1+1,y,z) );
		            	var g = data.getUint8( this.getId(pos-1+1,y,z)+gOff );
		            	var b = data.getUint8( this.getId(pos-1+1,y,z)+bOff );
		            	var index = 4 * (z * imageData.width + y);
		            	this.setImgData( index, r, g, b );
		            }
			}
			...
			this.setImgData = function( id, r, g, b ) {
				imageData.data[id] = r;
		...
		this.getImageGrayFloat = function(orient, pos) {
			...
			if ( orient === 'axial' ) {
				for( var x = 0; x < dimX; ++x )
		            for( var y = 0; y < dimY; ++y )
		            {
		            	var col = data.getFloat32( this.getIdFloat(x,(dimY-1)-y,pos) );
		            	var index = 4 * (y * imageData.width + x);
		            	this.setImgData( index, col );
		            }
			}
			...
			if ( orient === 'coronal' ) {
				for( var x = 0; x < dimX; ++x )
		            for( var z = 0; z < dimZ; ++z )
		            {
		            	var col = data.getFloat32( this.getIdFloat(x,pos,(dimZ-1)-z) );
		            	var index = 4 * (z * imageData.width + x);
		            	this.setImgData( index, col );
		            }
			}
			...
			if ( orient === 'sagittal' ) {
				for( var y = 0; y < dimY; ++y )
		            for( var z = 0; z < dimZ; ++z )
		            {
		            	var col = data.getFloat32( this.getIdFloat(pos,y,(dimZ-1)-z) );
		            	var index = 4 * (z * imageData.width + y);
		            	this.setImgData( index, col );
		            }
			}
			...
			this.setImgData = function( id, col ) {
